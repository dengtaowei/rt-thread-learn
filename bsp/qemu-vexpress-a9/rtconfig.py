...
if PLATFORM == 'gcc':
    ...
    CXXFLAGS= DEVICE + CFPFLAGS + ' -Wall -fdiagnostics-color=always -DO_DIRECTORY=0'
    CFLAGS  = DEVICE + CFPFLAGS + ' -Wall -Wno-cpp -std=gnu99 -D_POSIX_SOURCE -fdiagnostics-color=always -DO_DIRECTORY=0'
    AFLAGS  = DEVICE + ' -c' + AFPFLAGS + ' -x assembler-with-cpp'    
    LFLAGS  = DEVICE + ' -Wl,--gc-sections,-Map=rtthread.map,-cref,-u,system_vectors -T '+ LINK_SCRIPT + ' -lsupc++ -lgcc -static --specs=nosys.specs'
    CPATH   = ''
    LPATH   = ''
